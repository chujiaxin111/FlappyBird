...
void GameWindow::initControl()
{
    ...
    PipeItem *pipeItem = new PipeItem(m_scene); //生成管道
    startWelcome();
}
...
void GameWindow::initBackgroundMusic()
{
    ...
    m_welcomePlayerList->addMedia(QUrl("qrc:/BirdGame/Resources/sound/fade.mp3"));
    //...添加其他播放音乐
...
void GameWindow::startWelcome()
{
    ...
    connect(m_checkGameStatusTimer,SIGNAL(timeout()),
            this,SLOT(onCheckGameStatus()));//定时检测游戏状态
   ...
   {"飞", -1000, -1000, 250, 100 },//第一行
    ...
    btnItem->setPos(QPointF(180,320));
    ...
    connect(fadeAnimation,&QPropertyAnimation::finished,
        ...
        m_welcomePlayerList->addMedia(QUrl("qrc:/BirdGame/Resources/sound/bg.mp3"));
...
void GameWindow::GameOver()
{
    ...
    struct{
      ...
      {"您", -1000, -1000, 200, 160 },//第一行
    {"是", -800, -1000, 250, 160 },
    {"否", -600, -1000, 300, 160 },
    {"选", -400, -1000, 350, 160 },//第二行
    {"择", 1000, 2000, 400, 160 },
    {"重", 800, 2000, 225, 220 },
    {"新", 600, 2000, 275, 220 },
    {"开", 400, 2000, 325, 220 },
    ...
    //我从这儿开始胡乱写了
    QPixmap pix = QPixmap(":/BirdGame/Resources/image/restartButton.png");
    ButtonItem* btnItem1 = new ButtonItem(pix,m_scene);
    btnItem1->setPos(QPointF(180,320));

    //按钮渐渐消失动画
    QPropertyAnimation *fadeAnimation = new QPropertyAnimation(btnItem1,"opacity",m_lettersGroupFading);
    fadeAnimation->setDuration(600);//动画时长600ms
    fadeAnimation->setEndValue(0);//opacity为0就全透明，1为完全不透明
    fadeAnimation->setEasingCurve(QEasingCurve::OutQuad);
    connect(btnItem1,SIGNAL(clicked()),this,SLOT(onRestartBtnClicked()));//点击按钮
}
